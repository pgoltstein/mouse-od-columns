...
This script loads processed data of a L4 clusters and analyzes ODI as function of cluster center distance, e.g. including shuffles
...
parser = argparse.ArgumentParser( description = "This script loads processed data of a L4 clusters and analyzes ODI as function of cluster center distance, e.g. including shuffles.\n (written by Pieter Goltstein - Oct 2021)")
parser.add_argument('dataset', type=str, help= 'batch of mice to include ("b6-gcamp6s", "scn1a-jrgeco1a")')
...
# Select mice
if args.dataset == "b6-gcamp6s":
    mice = ["O02","O03","O06","O07","O09","O10","O11","O12","O13"]
    figname = "Fig-1"
elif args.dataset == "scn1a-jrgeco1a":
    mice = ["O38","O40","O41","O42","O43","O44","O45"]
    figname = "Fig-S7"
n_mice = len(mice)
...
# Actual data
datafile = np.load(os.path.join(processeddatapath,"cluster-odi-mice-{}.npz".format(args.dataset)))
odi_bins, n_clusters = datafile["odi_bins"], datafile["n_clusters"]

# Data with shuffled ODI
datafile_sh = np.load(os.path.join(processeddatapath,"cluster-odi-mice-{}-shuffled_odi.npz".format(args.dataset)))
odi_bins_sh, n_clusters_sh = datafile_sh["odi_bins"], datafile_sh["n_clusters"]

# Data with uniform XY
datafile_u = np.load(os.path.join(processeddatapath,"cluster-odi-mice-{}-uniform_xy.npz".format(args.dataset)))
odi_bins_u, n_clusters_u = datafile_u["odi_bins"], datafile_u["n_clusters"]

# Data with randomized clusters
datafile_r = np.load(os.path.join(processeddatapath,"cluster-odi-mice-{}-randomized_clusters.npz".format(args.dataset)))
...
savefile = os.path.join(savepath, figname+"g-odi-bins-indiv-mice-{}".format(args.dataset))
...
plottingtools.finish_panel( ax, title="", ylabel="ODI", xlabel="Position", legend="off", y_minmax=[-0.4,0.6], y_step=[0.2,1], y_margin=0.0, y_axis_margin=0.0, x_minmax=[0,1.01], x_margin=0.4, x_axis_margin=0.2, x_ticks=xvalues_2bin, x_ticklabels=["In","Out"] )
savefile = os.path.join(savepath, figname+"g-odi-in-out-cluster-{}".format(args.dataset))
plottingtools.finish_figure( filename=savefile, wspace=0.5, hspace=0.5 )
...
plottingtools.line( xvalues, odi_mn[swap_dist_nr,:], e=odi_sem[swap_dist_nr,:], line_color='#000000', line_width=1, sem_color=None, shaded=False )
plottingtools.finish_panel( ax, title="", ylabel="ODI", xlabel="Distance from cluster center (micron)", legend="off", y_minmax=[-0.4,0.6], y_step=[0.2,1], y_margin=0.0, y_axis_margin=0.0, x_minmax=[0.0,distance_range[1]+20.01], x_step=[100.0,0], x_margin=15, x_axis_margin=5 )
savefile = os.path.join(savepath, figname+"h-odi-bins-global-controls-{}".format(args.dataset))
plottingtools.finish_figure( filename=savefile, wspace=0.5, hspace=0.5 )
...
savefile = os.path.join(savepath, figname+"h-odi-in-cluster-vs-global-controls-{}".format(args.dataset))
plottingtools.finish_figure( filename=savefile, wspace=0.5, hspace=0.5 )
...
savefile = os.path.join(savepath, figname+"i-odi-bins-local-controls-{}".format(args.dataset))
plottingtools.finish_figure( filename=savefile, wspace=0.5, hspace=0.5 )
...
savefile = os.path.join(savepath, figname+"i-odi-in-cluster-vs-local-controls-{}".format(args.dataset))
plottingtools.finish_figure( filename=savefile, wspace=0.5, hspace=0.5 )
