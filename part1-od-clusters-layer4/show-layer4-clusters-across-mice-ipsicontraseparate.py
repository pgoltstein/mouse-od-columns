...
This script loads data of a single L4 imaging volume of all mice, finds clusters and calculates ODI for clusters, based on only ipsi preferring neurons and only contra preferring neurons

python show-layer4-clusters-across-mice-ipsicontraseparate.py
...
# Loop blocks of ipsi and contra cells only
for select_eye in ['ipsi','contra']:
    ...
    for m_nr,mouse in enumerate(mice):
        ...
        # Only select ipsi / contra -neurons
        XY_clust = np.array(XY)
        ODI_clust = np.array(ODI)
        if select_eye == "ipsi":
            XY = XY[ODI<=0,:]
            ODI = ODI[ODI<=0]
        else:
    ...
    if select_eye == "ipsi":
        title = "Ipsi-eye preferring cells"
    else:
        title = "Contra-eye preferring cells"
    ...
    plt.plot( [xvalues[test_bins[0]]-5,xvalues[test_bins[1]-1]+5], [0.75, 0.75], color="#000000", linewidth=1 ) # -1 to correct for list range end
    plt.plot( [xvalues[ctrl_bins[0]]-5,xvalues[ctrl_bins[1]-1]+5], [0.75, 0.75], color="#000000", linewidth=1 ) # -1 to correct for list range end
    plottingtools.finish_panel( ax, title=title, ylabel="ODI", xlabel="Distance from cluster center (micron)", legend="off", y_minmax=[-0.6,0.8], y_step=[0.2,1], y_margin=0.0, y_axis_margin=0.0, x_minmax=[0.0,distance_range[1]+20.01], x_step=[100.0,0], x_margin=15, x_axis_margin=5 )
    savefile = os.path.join(savepath, "Fig-S4g-odi-bins-indiv-mice-{}-cells-only".format(select_eye))
    ...
    plottingtools.finish_panel( ax, title=title, ylabel="ODI", xlabel="Position", legend="off", y_minmax=[-0.6,0.8], y_step=[0.2,1], y_margin=0.0, y_axis_margin=0.0, x_minmax=[0,1.01], x_margin=0.4, x_axis_margin=0.2, x_ticks=xvalues_2bin, x_ticklabels=["In","Out"] )
    savefile = os.path.join(savepath, "Fig-S4g-odi-in-out-cluster-{}-cells-only".format(select_eye))
    plottingtools.finish_figure( filename=savefile, wspace=0.5, hspace=0.5 )

    # Statistics
    print("\n\n--------\nTest versus control range --{}-- eye only:\n".format(select_eye))
