...
This script loads data of a single L4 imaging volume of all mice, finds clusters and calculates fraction of ipsi-preferring cells for clusters
...
parser = argparse.ArgumentParser( description = "This script loads data of a single L4 imaging volume of all mice, finds clusters and calculates fraction of ipsi-preferring cells for clusters.\n (written by Pieter Goltstein - Dec 2024)")
...
def fraction_ipsi_per_shell_f_0(XY_, ODI_, clusters_, bin_size_, distance_range_):
    XY_ipsi_ = XY_[ODI_<=0,:]
    count_all_per_bin,xvalues = densityclustering.count_per_shell(XY_[:,0], XY_[:,1], clusters_, bin_size=bin_size_, start=distance_range_[0], end=distance_range_[1])
    count_ipsi_per_bin,xvalues = densityclustering.count_per_shell(XY_ipsi_[:,0], XY_ipsi_[:,1], clusters_, bin_size=bin_size_, start=distance_range_[0], end=distance_range_[1])
    fraction_ipsi = count_ipsi_per_bin/count_all_per_bin
    return fraction_ipsi, xvalues


def fraction_ipsi_per_shell_f_neg03(XY_, ODI_, clusters_, bin_size_, distance_range_):
    XY_ipsi_ = XY_[ODI_<=-0.3,:]
    count_all_per_bin,xvalues = densityclustering.count_per_shell(XY_[:,0], XY_[:,1], clusters_, bin_size=bin_size_, start=distance_range_[0], end=distance_range_[1])
    count_ipsi_per_bin,xvalues = densityclustering.count_per_shell(XY_ipsi_[:,0], XY_ipsi_[:,1], clusters_, bin_size=bin_size_, start=distance_range_[0], end=distance_range_[1])
    fraction_ipsi = count_ipsi_per_bin/count_all_per_bin
    return fraction_ipsi, xvalues
...
for m_nr,mouse in enumerate(mice):
    ...
    # Get significance of neurons that are tuned
    if args.fractionipsi == "sign":
        onehot_odisigndiff = np.zeros((n_neurons,))
        for n in range(n_neurons):
            eye1_stimtrials = tuningmatrix[n,0,:,:].ravel() # Ipsi
            eye2_stimtrials = tuningmatrix[n,1,:,:].ravel() # Contra
            p,U,r,n1,n2 = statstools.mann_whitney_u_test( eye1_stimtrials, eye2_stimtrials )
            if (p < 0.05) and (np.nanmean(eye1_stimtrials) > np.nanmean(eye2_stimtrials)):
                onehot_odisigndiff[n] = 1
            ...
            if swap_dist > 0:
                ...
                for it in range(n_swap_iters):
                    ...
                    if args.fractionipsi == "0":
                        bin_v,_ = fraction_ipsi_per_shell_f_0(XY_sct, ODI, sct_clusters, bin_size, distance_range)
                    elif args.fractionipsi == "-0.3":
                        bin_v,_ = fraction_ipsi_per_shell_f_neg03(XY_sct, ODI, sct_clusters, bin_size, distance_range)
                    elif args.fractionipsi == "sign":
                        bin_v,_ = densityclustering.value_per_shell(XY_sct[:,0], XY_sct[:,1], onehot_odisigndiff, sct_clusters, bin_size=bin_size, start=distance_range[0], end=distance_range[1])
                ...
                # Get swap_dist bins
                if args.fractionipsi == "0":
                    bin_values, xvalues = fraction_ipsi_per_shell_f_0(XY, ODI, clusters, bin_size, distance_range)
                elif args.fractionipsi == "-0.3":
                    bin_values, xvalues = fraction_ipsi_per_shell_f_neg03(XY, ODI, clusters, bin_size, distance_range)
                elif args.fractionipsi == "sign":
                    bin_values, xvalues = densityclustering.value_per_shell(XY[:,0], XY[:,1], onehot_odisigndiff, clusters, bin_size=bin_size, start=distance_range[0], end=distance_range[1])
                bin_values = np.nanmean(bin_values,axis=0)
...
# Save data
if args.fractionipsi == "0":
    savefile = os.path.join(processeddatapath, add_suffix("cluster-fractionipsi-smallerthan0-mice", args, ".npz") )
elif args.fractionipsi == "-0.3":
    savefile = os.path.join(processeddatapath, add_suffix("cluster-fractionipsi-smallerthanminus0point3-mice", args, ".npz") )
elif args.fractionipsi == "sign":
    savefile = os.path.join(processeddatapath, add_suffix("cluster-fractionipsi-significantipsicontra-mice", args, ".npz") )

np.savez(savefile, frac_bins=swap_bin_values, n_clusters=n_clusters_detected, n_swap_dist_exceeds=n_swap_dist_exceeds, n_swap_dist_underceeds=n_swap_dist_underceeds)
